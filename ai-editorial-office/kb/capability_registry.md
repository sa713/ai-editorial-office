...
Task Need Recognition is a shared advisory capability, not a standing Task
Router, Classifier, Triage Agent, pipeline, gate, score, or automatic routing
system. Its evidence-first request-to-need contract lives in
`/kb/task_need_recognition.md`; Chief Editor retains every decision.
...
- Purpose: turn available request evidence into an advisory view of likely task
  type, outcome-first single or minimal deliverable set, capabilities, Domain Packs,
...
- Typical outputs: compact recognition view in `brief.md`,
  ...
  recommendations including the minimum sufficient outcome-fit artifact family,
...
- Accountability wrapper: Intake Agent normally assembles the initial view;
  Chief Editor challenges it and owns every selected-deliverable-set/routing/
...
- Stop conditions: keyword-only recommendation, forced single type despite
  ...
  deliverable-set conflation, silent explicit-format expansion or override,
  purposeless companion, duplicate owner, automatic action, score/threshold
  authority, or unjustified process weight.
- Quality criteria: observed evidence is separate from recommendation and Chief
  ...
  owner-referenced, qualitative, uncertainty-aware, and explicitly advisory.
- Expansion triggers: unclear task type, material mixed task, high-governance
  risk, multiple capability or Domain Pack candidates, architecture/engineering
  significance, conflicting evidence, or decomposition uncertainty.

### Routing And Preflight

- Purpose: decide whether one artifact is sufficient, choose an ordered minimal
  selected deliverable set from the requested deliverable and advisory
  recommendation, then choose task type, primary pipeline/mode and bounded
...
- Stop conditions: risk mode unknown before production, missing critical input,
  ...
  conflict.
- Quality criteria: requested deliverable, recommended deliverable set, and
  selected deliverable set are distinct; explicit user intent is preserved;
  one-artifact sufficiency is tested; every selected member has a distinct
  purpose, dependency, and production priority; the set is minimal and
  sufficient for the actual outcome; pipeline follows the set decision; route
  is deliberate, compact when safe, expanded when needed, and review gate is
  preserved.
